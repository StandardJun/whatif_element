...
- [x] **포스트 이미지 고도화** (384개 포스트별 고유 이미지/alt 텍스트, 커밋 `2958f92`)
- [x] **매칭 점수 ⓘ 툴팁** 추가 (hover/touch, 한영 지원, 커밋 `a7100dc`)
- [x] **결과 이미지 캡처 수정**: 카테고리 배지/설명 정렬 fix + 성격지표·궁합 캡처 영역 포함 (커밋 `c071c38`)
- [x] **캡처 이미지 추가 정렬 fix** + 도감 궁합 위치 이동 + 결과 궁합 클릭 가능 (커밋 `5fc82de`)
- [x] **118개 원소 전체 결과 도달 가능**: 매칭 가중치(0.6/0.4) + 39개 원소 traits 미세 조정 (커밋 `8c01532`)
- [x] **배포 전 콘텐츠 리뷰**: 118개 원소 전수 검토 → 20개 항목 수정 (불쾌 표현 16, 과학 부정확 2, 매칭 오류 1, 톤 조정 1) (커밋 `5272907`)
- [x] **2차 콘텐츠 리뷰**: 118개 원소 재검토 → 22개 항목 수정 (불쾌 표현 4, 톤 불균형 9, 과학 부정확 1, 부정적 뉘앙스 8). 6배치 병렬 sonnet 에이전트 활용. 빌드 확인 (598페이지) (커밋 `bde8877`)
- [x] **커스텀 favicon**: 기본 Next.js 아이콘 → 오렌지→핑크 그라데이션 "?" SVG 아이콘 (커밋 `0548a54`)
- [ ] 도메인 연결 (manyak.xyz)
...
| 2026-02-07 | 배치 프로토콜로 콘텐츠 리뷰 | 118개 원소를 6배치(20개씩)로 나눠 sonnet 병렬 검토. 파일럿→웨이브 패턴 |
| 2026-02-07 | 불쾌 표현 기준 | "독성/위험/폭발/치명적/독약" 등 사용자를 부정적으로 묘사하는 원소 특성 비유 제거 |
| 2026-02-07 | 2차 리뷰 기준 확장 | 1차 기준(독성/위험/폭발) + 어색한 한국어, 톤 불균형, 미묘한 부정 뉘앙스 추가 |
| 2026-02-07 | 수소폭탄→핵융합 실험 | Es(99), Fm(100)의 '수소폭탄' 표현을 과학적·중립적 '핵융합 실험'으로 통일 |
...
4. ✅ **포스트 이미지 고도화** (커밋 `2958f92`)
5. ✅ **중복 푸터 제거** — layout.tsx 글로벌 푸터만 남기고 4개 로컬 푸터 삭제
6. ✅ **모바일 이미지 저장** — Web Share API(`navigator.share files`) 방식 (iOS/Android 네이티브 공유 시트)
7. ✅ **캡처 텍스트 정렬** — html2canvas → `html-to-image` 라이브러리 교체 (SVG foreignObject 네이티브 렌더링)
8. ✅ **버튼 UX 개선** — 사진 저장 버튼 메인 CTA화, 테스트 링크 공유 텍스트 명확화 + 연한 그라데이션
9. ✅ **포스트 렌더링 개선** — 마크다운 파서 전면 개선 (이미지+캡션, 테이블, 이탤릭, 번호리스트, 텍스트 크기 조정) (커밋 `301f31c`)
10. ✅ **깨진 이미지 URL 수정** — 3개 (스페이스 제거 1, 404 URL 교체 2) (커밋 `301f31c`)
11. ⏳ **이미지 중복 교체** — 98개 photo ID가 크로스포스트 중복 (최악: 1개 사진→16개 포스트). 감사 보고서 완성, 교체 작업 미진행
12. ⏳ **포스트 팩트체크** — 354개 포스트 전수 검증 필요 (이전 sub-agent 작업 신뢰도 낮음)
13. ⏳ 도메인 연결 (manyak.xyz)
14. ⏳ AdSense 재심사 요청
15. ⏳ 정식 문의 이메일 생성 (현재 임시: jkgkgj@naver.com)
16. ⏳ **영어 도감 번역**: 영어 모드 시 도감(/elements) 페이지가 한국어로 표시됨. 도감 완성 후 번역 작업 필요 (elementExtras.ts, posts 콘텐츠 등)
